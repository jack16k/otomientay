...
<div class="container">
	<div class="row" style="background-color: #fff; margin: 0px;">
        ...
        <div class="col-sm-6 text-right">
	        <a class="my-btn-primary" style="color: #fff" href="{{URL('quantri/posts/add')}}" >
					...
					<tbody>
						@foreach($posts as $post)
							<tr>
								...
								<td>
				                    <input id="cb{{$stt++}}" name="cid[]" value="{{$post->p_id}}" onclick="isChecked(this.checked);" type="checkbox">
				                </td>
				                <td>
				                    <img src="{{$post->p_image}}" style="width: 30px; height: 30px" title="Ảnh sai đường dẫn"/>
				                </td>
				                <td style="color: #337ab7;">
				                @php echo $post->p_title; @endphp
				                </td>
				                <td>
				                {{$post->p_alias}}
				                </td>
				                <td>
				                @php echo $post->p_description; @endphp
				                </td>
				                
				                <td align="center"> 
				                	<a href="{{URL('quantri/posts/edit').'/'.$post->p_id}}" class="">
				                        <i class="glyphicon glyphicon-edit"></i>
				                    </a>
				                </td>
				                <td align="center"> 
				                	<a href="#" style="color: #c12e2a;" onclick="deleteCategory({{$post->p_id}})">
